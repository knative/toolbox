...
import (
	...
	"knative.dev/toolbox/go-ls-tags/cli"
	"knative.dev/toolbox/go-ls-tags/tags"
	"knative.dev/toolbox/go-ls-tags/test"
)
...
func (tc *testCase) test(t *testing.T) {
	...
	if tc.err != nil {
		if !(errors.Is(tc.err, te.err) || tc.err.Error() == te.err.Error()) {
			t.Fatalf("want error: %#v, got: %#v", tc.err, te.err)
		}
	} else {
		if te.err != nil {
			t.Fatalf("want no error, got: %#v", te.err)
		}
	}
	text := te.out.String()
